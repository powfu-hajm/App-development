...
public class TestController {
    ...
    public Result<TestResultDTO> submitTest(@Valid @RequestBody TestSubmitDTO submitDTO) {
        try {
            log.info("提交测试答案，paperId: {}, userId: {}",
                    submitDTO.getPaperId(), submitDTO.getUserId());
     ...
     */
    @GetMapping("/history")
    public Result<List<TestResultDTO>> getTestHistory(@RequestParam Long userId) {
        try {
            log.info("获取用户测试历史，userId: {}", userId);
            List<TestResultDTO> history = testService.getTestHistory(userId);
